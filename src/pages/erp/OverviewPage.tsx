...
type SubmissionMin = {
  ...
  ai_score: { overall: number } | null; created_at: string
...
function useStores() {
  return useQuery<StoreMin[]>({
    queryKey: ['stores'],
    queryFn: async () => {
      const { data, error } = await supabase.from('stores').select('id, name, region')
      if (error) throw error
  ...
  return useQuery<SubmissionMin[]>({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select('id, status, ai_score, created_at, staff:staff(name), store:stores(name), theme:themes(title)')
  ...
  return useQuery<number>({
    ...
    queryFn: async () => {
      const { count, error } = await supabase
        .from('roleplay_submissions').select('id', { count: 'exact', head: true }).gte('created_at', weekStart)
      if (error) throw error
      return count ?? 0
    },
    staleTime: 60_000,
...
const STATUS_CFG: Record<SubmissionMin['status'], { label: string; className: string }> = {
  submitted:   { label: 'Submitted',   className: 'bg-blue-50 text-blue-700 border-blue-200 hover:bg-blue-50' },
  ai_reviewed: { label: 'AI Reviewed', className: 'bg-amber-50 text-amber-700 border-amber-200 hover:bg-amber-50' },
...
export default function OverviewPage() {
  ...
  const { data: tasks = [], dataUpdatedAt: tasksTs, isLoading: tL } = useTasks()
  const { data: stores = [], isLoading: sL } = useStores()
  const { data: submissions = [], isLoading: subL } = useRecentSubmissions()
  const { data: weekCount = 0 } = useWeeklyCount()
  const today = todayStr()
  const isLoading = tL || sL || subL

  const totalPending = useMemo(() => tasks.filter((t) => t.status === 'pending').length, [tasks])
  const dueToday = useMemo(() => tasks.filter((t) => t.status === 'pending' && t.due_date === today).length, [tasks, today])
  const avgCompletion = useMemo(() => {
    ...
    for (const s of stores) {
      ...
      sum += (st.filter((t) => t.status === 'done').length / st.length) * 100; counted++
    }
  ...
  const barData = useMemo(() => {
    ...
    for (const t of tasks) if (t.status === 'pending') counts.set(t.store_id, (counts.get(t.store_id) ?? 0) + 1)
  ...
  const storeRows = useMemo(() =>
    stores.map((s) => {
      const st = tasks.filter((t) => t.store_id === s.id)
      const lastSync = st.map((t) => t.updated_at).filter((v): v is string => v !== null).sort().pop() ?? null
      return { ...s, pending: st.filter((t) => t.status === 'pending').length, done: st.filter((t) => t.status === 'done').length, dueToday: st.filter((t) => t.status === 'pending' && t.due_date === today).length, lastSync }
    }), [tasks, stores, today])

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <p className="text-xs text-muted-foreground">{tasksTs ? `Last updated ${fmtTime(tasksTs)}` : ' '}</p>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <Card>
          <CardHeader className="pb-0"><CardTitle className="text-sm font-semibold">Tasks by Store</CardTitle><p className="text-xs text-muted-foreground">Pending count — top 10 stores</p></CardHeader>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <Card>
          ...
          <CardContent className="px-0 pb-1">
            <Table>
              <TableHeader><TableRow><TableHead className="pl-5">Store</TableHead><TableHead>Region</TableHead><TableHead className="text-right">Pending</TableHead><TableHead className="text-right">Due Today</TableHead><TableHead className="text-right">Done</TableHead><TableHead className="pr-5">Last Sync</TableHead></TableRow></TableHeader>
              <TableBody>
                {storeRows.length === 0 ? <EmptyRow cols={6} message="No stores found" /> : storeRows.map((row) => (
                  <TableRow key={row.id} className="cursor-pointer" onClick={() => navigate(`/erp/upload?store=${row.id}`)}>
                    ...
                    <TableCell className="text-right"><span className={row.pending > 0 ? 'font-semibold text-orange-600' : 'text-muted-foreground'}>{row.pending}</span></TableCell>
                    <TableCell className="text-right"><span className={row.dueToday > 0 ? 'font-semibold text-red-600' : 'text-muted-foreground'}>{row.dueToday}</span></TableCell>
                    <TableCell className="text-right text-muted-foreground">{row.done}</TableCell>
                    <TableCell className="pr-5 text-xs text-muted-foreground">{fmtDate(row.lastSync)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
        ...
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-semibold">Recent Roleplay Submissions</CardTitle></CardHeader>
          <CardContent className="px-0 pb-1">
            <Table>
              <TableHeader><TableRow><TableHead className="pl-5">Staff</TableHead><TableHead>Store</TableHead><TableHead>Theme</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Score</TableHead><TableHead className="pr-5">Submitted</TableHead></TableRow></TableHeader>
              <TableBody>
                {submissions.length === 0 ? <EmptyRow cols={6} message="No submissions yet" /> : submissions.map((sub) => (
                  <TableRow key={sub.id}>
                    ...
                    <TableCell className="text-right">{sub.ai_score?.overall != null ? <span className="font-semibold text-gray-900">{sub.ai_score.overall.toFixed(1)}</span> : <span className="text-muted-foreground">–</span>}</TableCell>
